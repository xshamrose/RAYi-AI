 ...
 const sendMessage = () => {
   ...
   const fetchData = async (userMessage) => {
    try {
        ...
        const response = await fetch(`your-api-url?message=${encodeURIComponent(userMessage)}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
        setFetchedData(data.response); // Adjust this line based on the actual structure of your API response
     } catch (error) {
