 ...
 const handleSubmit = () => {
    if (!imageFile) {
      ...
      if (title === "Breast Cancer Detector") {
        apiUrl = "http://ct.rayi.in:5002/breastCancerDetect"; // Example URL for Breast Cancer Detector
      } else if (title === "Face Detector") {
        apiUrl = "http://ct.rayi.in:5000/recognize"; // Example URL for Face Detector
      }
      ...
      axios
        ...
        .then((response) => {
          setLoading(false);
          setResponseData(JSON.stringify(response.data, null, 2)); // Convert to formatted JSON string for display
        })
        .catch((error) => {
          setLoading(false);
            ...
            <Card title="Output" hoverable>
              <div className="text-area">
                {loading ? (
                 ...
                 <pre>{responseData}</pre> // Use <pre> to preserve formatting
                )}
              </div>
            </Card>
          </div>
