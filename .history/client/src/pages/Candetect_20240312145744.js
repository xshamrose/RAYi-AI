...
console.log(totalCount)
 ...
 const getModelContent = () => {
  if (title === "Breast Cancer Detector") {
     return (
       <div>
         <span>Overview: Our breast cancer detection model utilizes state-of-the-art deep learning techniques to analyze histopathological images of breast tissue and provide accurate predictions regarding the presence or absence of cancerous cells.</span>
         <p>How it Works: Users upload histopathological images of breast tissue to the web interface for analysis. Leveraging convolutional neural networks (CNNs) trained on vast datasets of annotated breast tissue images, the model processes these images with intricate pattern recognition techniques.</p>
       </div>
     );
  } else if (title === "Face Detector") {
     return (
       <div>
         <p>Overview: Our cutting-edge face recognition model is designed to accurately identify individuals from images, empowering users with seamless identity verification capabilities.</p>
         <p>How it Works: Using our intuitive web interface, users can upload images containing faces for analysis. Our model swiftly processes these images, identifying individuals with high accuracy and providing detailed recognition results.</p>
       </div>
     );
  } else if (title === "Pipe Counting") {
     return (
       <div>
         <p>Overview: Our pipe counting model utilizes advanced computer vision algorithms to accurately detect and count pipes within images.</p>
         <p>How it Works: Users can upload images containing pipe networks to the web interface. The model employs sophisticated image processing techniques, including object detection and instance segmentation, to identify individual pipes within the scene.</p>
       </div>
     );
  }
  return null; // Default case, no content to display
 };
