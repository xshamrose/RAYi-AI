...
function Candetect() {
 ...
 const handleSubmit = () => {
  if (!imageFile) {
     ...
     if (title === "Breast Cancer Detector") {
       ...
       // For Breast Cancer Detector, continue using FormData with the default key
       const formData = new FormData();
       formData.append("image", imageFile);
       makeApiCall(apiUrl, formData);
     } else if (title === "Face Detector") {
       apiUrl = "http://ct.rayi.in:5000/recognize";
       // For Face Detector, use FormData with the key "image_base64"
       const formData = new FormData();
 ...
 const makeApiCall = (apiUrl, formData) => {
  axios
